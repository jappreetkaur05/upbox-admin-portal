  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description="Verify FEs at the warehouse gate with OTP confirmation."
      />
      ...
      <section className="surface-card mb-6 p-5">
        ...
        <ul className="mt-3 space-y-3">
          ...
          {!pending.length ? (
            <p className="text-sm text-slate-500">No pending FE check-ins.</p>
          ) : null}
        </ul>
      </section>
      ...
      <section className="surface-card p-5">
        ...
        <ul className="mt-3 space-y-2">
          {verified.map((c) => (
            <li key={c.id} className="flex items-center justify-between rounded-xl border border-emerald-100 bg-emerald-50/50 px-4 py-2 text-sm">
              <div>
                <p className="font-semibold">{c.feName}</p>
          ...
          {!verified.length ? (
            <p className="text-sm text-slate-500">No verified check-ins yet.</p>
          ) : null}
        </ul>
      </section>
    </div>
