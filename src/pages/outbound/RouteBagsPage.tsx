...
export function RouteBagsPage() {
  ...
  const routesQ = useDeliveryRoutes()
  ...
  const waitingReady = useMemo(
    () => (ordersQ.data ?? []).filter((o) => o.status === 'READY' && o.routeId),
    [ordersQ.data]
  )

  const waitingByRoute = useMemo(() => {
    const map = new Map<string, OutboundOrder[]>()
    for (const o of waitingReady) {
      const key = o.routeId!
  ...
  const routeName = (routeId: string) =>
    routesQ.data?.find((r) => r.id === routeId)?.name ?? routeId
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Route bags"
        description="See READY orders waiting to sort, then seal bags for FE handoff."
        actions={
          <button
            type="button"
            className="rounded-xl bg-primary-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
            disabled={sortReady.isPending || !waitingReady.length}
            onClick={async () => {
              try {
      ...
      <OutboundChrome
        what="Group READY orders by delivery route into sealed bags."
        doNow={
          waitingReady.length
            ? `${waitingReady.length} order(s) across ${waitingByRoute.size} route(s) waiting to sort.`
            : 'No READY orders waiting — finish packing first.'
        }
        nextLabel="Next: FE check-in"
        nextTo="/outbound/fe-checkin"
      >
        <section className="surface-card mb-6 p-5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="font-heading text-base">Waiting to sort</h2>
            <p className="text-xs text-slate-500">
              {waitingReady.length} orders · {waitingByRoute.size} routes ready to bag
            </p>
          </div>
          {waitingReady.length ? (
            <div className="mt-4 space-y-4">
              {[...waitingByRoute.entries()].map(([routeId, orders]) => (
                <div key={routeId}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                    {routeName(routeId)} · {orders.length} order(s)
                  </p>
            ...
            <p className="mt-3 text-sm text-slate-500">
              ...
              <Link className="font-semibold text-primary-700 underline" to="/outbound/packing">
                finish packing
              </Link>{' '}
              first.
            </p>
              ...
              <section key={bag.id} className="surface-card overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {bag.routeCode} · <span className="font-mono">{bag.bagBarcode}</span> · {bag.orderIds.length}{' '}
                      orders
                    </p>
                  </div>
                    ...
                    {bag.orderIds.map((oid) => {
                      ...
                      return (
                        <li key={oid} className="flex items-center justify-between px-4 py-3 text-sm">
                          ...
                          {order ? <StatusBadge label={order.status} tone="slate" /> : null}
                        </li>
                      )
                    })}
                    {!bag.orderIds.length ? (
