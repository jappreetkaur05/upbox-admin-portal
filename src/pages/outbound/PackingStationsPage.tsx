...
type PackStep = 'validate' | 'qc' | 'print'
...
export function PackingStationsPage() {
  ...
  const allSessionCandidates = [
    ...packingOrders,
    ...(packedQ.data ?? []),
    ...(readyQ.data ?? []).filter((o) => o.id === justCompletedId),
  ]
  ...
  const sessionOrder =
    allSessionCandidates.find((o) => o.id === sessionOrderId) ??
    [...(packedQ.data ?? []), ...(readyQ.data ?? [])].find((o) => o.id === sessionOrderId) ??
    null
  ...
  const validated =
    !!sessionOrder &&
    (sessionOrder.weightValidated && sessionOrder.dimsValidated ||
      sessionOrder.status === 'PACKED' ||
      sessionOrder.status === 'READY')

  const isReady = sessionOrder?.status === 'READY'
  const step: PackStep = !sessionOrderId ? 'validate' : isReady ? 'print' : validated ? 'qc' : 'validate'
  ...
  const handlePrintLabel = async (orderId: string) => {
    try {
      ...
      printShippingLabel(label, sessionOrder)
  ...
  const steps: { id: PackStep; label: string }[] = [
    ...
    { id: 'qc', label: '2. Complete QC' },
    { id: 'print', label: '3. Print label' },
  ]
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description="Assign picked orders, validate weight/dims, complete QC, then print the shipping label."
      />

      <OutboundChrome
        what="Pack picked orders at a station."
        doNow={
          pickedCandidates.length
            ? `${pickedCandidates.length} picked order(s) waiting for a station.`
            : packingOrders.length
              ? 'Finish validate → QC → print on the active order.'
              : 'No packing work — pick more orders or go to route bags.'
        }
          ...
          {stations.map((s) => (
            <div key={s.id} className="surface-card p-4">
              <div className="flex items-center justify-between">
                ...
                <StatusBadge
                  label={s.status}
                  tone={s.status === 'BUSY' ? 'violet' : s.status === 'IDLE' ? 'green' : 'slate'}
                />
              </div>
            ...
            {!sessionOrderId ? (
              ...
              <>
                <p className="mt-1 text-xs text-slate-500">
                  Order {sessionOrder?.orderNumber ?? sessionOrderId}
                </p>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <label className="text-xs">
                    L cm
                    <input value={l} onChange={(e) => setL(e.target.value)} className="mt-1 w-full rounded-lg border px-2 py-1.5" disabled={step !== 'validate'} />
                  </label>
                  <label className="text-xs">
                    W cm
                    <input value={w} onChange={(e) => setW(e.target.value)} className="mt-1 w-full rounded-lg border px-2 py-1.5" disabled={step !== 'validate'} />
                  </label>
                  <label className="text-xs">
                    H cm
                    <input value={h} onChange={(e) => setH(e.target.value)} className="mt-1 w-full rounded-lg border px-2 py-1.5" disabled={step !== 'validate'} />
                  </label>
                </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (res.weightOk && res.dimsOk) {
                            toast('Validation passed — next: Complete QC', 'success')
                          } else {
                            toast('Validation failed — fix weight or dimensions', 'error')
                          }
                        } catch (e) {
                          toast((e as Error).message, 'error')
                        }
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setJustCompletedId(sessionOrderId)
                          toast('QC passed — next: Print shipping label', 'success')
                        } catch (e) {
                          toast((e as Error).message, 'error')
                        }
                      }}
